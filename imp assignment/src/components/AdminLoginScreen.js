...
export default function AdminLoginScreen() {
  ...
  let submit=() => {
    setloader(true)
    Login("Admin",{Email:Email,Password:Password}).then((succ)=> {
    setloader(false)
      navigate(`/AdminDashboard/${succ.User_Id}`)
    }).catch((err) => {
    setloader(false)
      console.log(err)})
  }
  return (
    ...
    <Box sx={{borderRadius:"19px",marginTop:"40px",marginLeft:"484px",position:"relative",height:"400px",width:"434px",backgroundColor:"#faebd7"}}>
      ...
      <Box>
        <Typography variant="h4" sx={{marginLeft:"100px",marginTop:"20px",fontFamily: '"Times New Roman", Times, serif',fontWeight:"bold",color:"#0047AB"}}>Admin Portal</Typography>
